        ...
        for property in contentType.properties {
            ...
            case .bool:
                guard let value = anyValue as? Bool else {
                    print(
                        "ERROR: property is not a bool (\(property.key): \(anyValue ?? "nil"))."
                    )
                    break
                }
                wrapper = .bool(value)
            case .int:
                guard let value = anyValue as? Int else {
                    print(
                        "ERROR: property is not an integer (\(property.key): \(anyValue ?? "nil"))."
                    )
                    break
                }
                wrapper = .int(value)
            case .double:
                guard let value = anyValue as? Double else {
                    print(
                        "ERROR: property is not a double (\(property.key): \(anyValue ?? "nil"))."
                    )
                    break
                }
            ...
            case .string:
                guard let value = anyValue as? String else {
                    print(
                        "ERROR: property is not a string (\(property.key): \(anyValue ?? "nil"))."
                    )
                    break
                }
            ...
            case let .date(format):
                ...
                guard let value = formatter.date(from: rawDateValue) else {
                    print(
                        "ERROR: property is not a date (\(property.key): \(anyValue ?? "nil"))."
                    )
                    break
                }
